...
def run(args):
    ...
    video_model = get_video_model(ckpts_dir=args.ckpt_dir, milestone=args.milestone, timestep=10)
    target_timestep = 100
    #target_video_model = get_video_model(ckpts_dir=args.ckpt_dir, milestone=args.milestone, timestep=100)
    ...
    for camera in cameras:
        ...
        for seed in tqdm(range(n_exps)):
            try: 
                ...
                obs = env.reset()
                policy = MyPolicy_CL(env, env_name, camera, video_model, target_timestep, flow_model, max_replans=max_replans)
