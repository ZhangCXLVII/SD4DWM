...
def pred_video(draft_model, target_timestep, frame_0, task, flow=False):
    ...
    text = [task]
    #ret, draft_tokens = model.sample(image.to(device), text)
    #preds = rearrange(ret.cpu().squeeze(0), "(f c) w h -> f c w h", c=channels)
    #preds = speculative_decoding(model, target_model, image.to(device), text)
    preds = rearrange(draft_model.sample(image.to(device), text, draft_model, target_timestep).cpu().squeeze(0), "(f c) w h -> f c w h", c=channels)
    if not flow:
