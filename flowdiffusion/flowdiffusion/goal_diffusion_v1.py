                ...
                if accelerator.is_main_process:
                    ...
                    if self.step != 0 and self.step % self.save_and_sample_every == 0:
                        ...
                        if self.step == self.save_and_sample_every:
                            ...
                            gt_img = torch.cat([x_starts, x_ends, x_conds, gt_xs[:, :]], dim=1)
                            gt_img = rearrange(gt_img, 'b n c h w -> (b n) c h w', n=n_rows+3)
                            utils.save_image(gt_img, str(self.results_folder / f'imgs/gt_img.png'), nrow=n_rows+3)

                        os.makedirs(str(self.results_folder / f'imgs/outputs'), exist_ok = True)
                        pred_img = torch.cat([x_starts, x_ends, x_conds, all_xs[:, :]], dim=1)
                        pred_img = rearrange(pred_img, 'b n c h w -> (b n) c h w', n=n_rows+3)
                        utils.save_image(pred_img, str(self.results_folder / f'imgs/outputs/sample-{milestone}.png'), nrow=n_rows+3)
